...
import pandas as pd
import numpy as np
from svd import SVDWordEmbeddings
import torch
...
embeddings_model_train.preprocess_text()
...
with open('./word_index_svd.pkl', 'rb') as file:
    word_index_svd = pickle.load(file)

word_vectors_svd = torch.load('./svd-word-vectors.pth')
...
model = torch.load('./svd-classification-model.pth')
